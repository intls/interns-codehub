...
pub fn swap(
    ...
    if a_to_b && sqrt_price_limit > globalpool.sqrt_price
        || !a_to_b && sqrt_price_limit < globalpool.sqrt_price
    {
    ...
    let fee_rate = globalpool.fee_rate;
    let protocol_fee_rate = globalpool.protocol_fee_rate;
        ...
        if swap_computation.next_price == next_tick_sqrt_price {
            ...
            if next_tick_initialized {
                let (fee_growth_global_a, fee_growth_global_b) = if a_to_b {
                    (curr_fee_growth_global_input, globalpool.fee_growth_global_b)
                } else {
                    (globalpool.fee_growth_global_a, curr_fee_growth_global_input)
                };
